...
> [!tip]
>此模块为自用，不代表原作者，也没有获得许可，测试设备y700gen2，仅尝试用AI增改横屏适配以及适配数量文本，（由于zux本身接口与完美横屏计划所用的澎湃横屏配置文件并不完全相符，尤其是大多数为miui11的适配文件，如强制横屏参数在zux上貌似是缺失的导致qq就算有配置适配，在群文件页面也会因应用自身原因会默认回到竖屏，故由完美横屏计划迁移补充的7000+适配其实绝大部分处于残缺状态），如有问题请自行解决且不要因本二改模块问题麻烦原作者。
>
> 
> ------------------下方为原作内容--------------------------
>
